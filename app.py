...
try:
    ...
    st.error(
        f"Please ensure 'Meesho dashboard.xlsx' is in the same folder "
        f"as app.py. Error: {e}"
    )
    st.stop()
...
def calculate_debit_mom(
    ...
    # Multiple month filter
    if sel_months:
        d = d[d['Month_Name'].isin(sel_months)]
...
if page == "SHORTAGE VIEW":
    ...
    if sel_months:
        filtered_df = filtered_df[
            filtered_df['Month_Name'].isin(sel_months)
        ]
    ...
    total_shortage_amount = filtered_df[
        'Total Amount'
    ].sum()
