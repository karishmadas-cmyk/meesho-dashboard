...
def load_data():
    ...
    # Clean Column Names
    shortage_df.rename(
        columns={'Assigned/ marked ': 'Assigned/ marked'},
        inplace=True
    )
...
try:
    ...
    st.error(
        f"Please ensure 'Meesho dashboard.xlsx' is in the same folder as app.py. "
        f"Error: {e}"
    )
...
# Helper function to auto-format Horizontal Bar Charts
...
def calculate_debit_mom(
    ...
    if sel_year != "All":
        ...
        monthly = monthly[
            monthly['Month'].dt.year.astype(str) == sel_year
        ]
    ...
    #
    # The latest selected month is treated as the current month.
    # MoM compares it with the immediately preceding calendar month.
    # -------------------------------------------------------------------------
    ...
    if sel_months:
        ...
        # Keep only selected months to determine latest selected month
        selected_rows = monthly[
            monthly['Month'].dt.month.isin(
                selected_month_numbers
            )
        ]
        ...
        # Latest selected month/date
        current_date = selected_rows['Month'].max()
        ...
        curr_val = current_row.iloc[0]['Total amount']
        ...
        prev_val = previous_row.iloc[0]['Total amount']
...
if page == "SHORTAGE VIEW":
    ...
    top_cat = (
        filtered_df['Assigned/ marked'].mode()[0]
    ...
    top_reason = (
        filtered_df['Shorage type'].mode()[0]
        if not filtered_df.empty
        else "N/A"
    )
    ...
    peak_month = (
        filtered_df
        .groupby('Month_Year')['Total Amount']
        .sum()
        .idxmax()
        if not filtered_df.empty
        else "N/A"
...
# =============================================================================
    ...
    if sel_year != "All":
        ...
        filtered_debit = filtered_debit[
            filtered_debit['Year'] == sel_year
        ]

        filtered_weekly = filtered_weekly[
            filtered_weekly['Year'] == sel_year
        ]
    ...
    if sel_month:
        ...
        filtered_debit = filtered_debit[
            filtered_debit['Month_Name'].isin(
                sel_month
            )
        ]

        filtered_weekly = filtered_weekly[
            filtered_weekly['Month_Name'].isin(
                sel_month
            )
        ]
    ...
    overall_debit_amount = (
        filtered_debit['Total amount'].sum()
    )

    weekly_shipment_count = len(
        filtered_weekly
    )

    weekly_debit_amount = (
        filtered_weekly['Value'].sum()
    )
    ...
    with c3:
        ...
        st.markdown(
            ...
            <div class="kpi-card">
                ...
                <div class="kpi-value">
                    {format_count(weekly_shipment_count)}
                </div>

                <div class="kpi-label">
                    Shipment count (Weekly Debit)
                </div>
    ...
    with col_l1:
        ...
        if not w_trend.empty:
            ...
            fig_w.update_traces(
                ...
                # Actual value displayed above each point
                text=w_trend['Value'].apply(
                    lambda x: f"₹{x:,.0f}"
                ),
            ...
            # Y AXIS
            #
            # Removed fixed dtick=2000000.
            # Plotly will automatically choose suitable intervals.
            # -----------------------------------------------------------------
    ...
    with col_l2:
        ...
        if not o_trend.empty:
            ...
            fig_o.update_traces(
                ...
                text=o_trend['Total amount'].apply(
                    lambda x: f"₹{x:,.0f}"
                ),
            ...
            st.info(
                "No overall debit data available for the selected filters."
            )
    ...
    st.markdown(
        "##### TOP 5 MONTHLY DEBIT CONTRIBUTORS"
    )
    ...
    if not filtered_debit.empty:
        ...
        st.info(
            "No debit data available for the selected filters."
        )
    ...
    with col_b1:
        ...
        loss_cat = (
            filtered_debit
            .groupby('Loss Type')['Total amount']
            .sum()
            .reset_index()
        )
    ...
    with col_b2:
        ...
        top_loss_loc = (
            ...
            filtered_debit['Location'].mode()[0]
